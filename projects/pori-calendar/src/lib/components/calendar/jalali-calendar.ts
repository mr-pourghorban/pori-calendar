...
import { PoriDays, PoriDaysAbbr } from '../../utils/days';
import { PoriDateModel } from '../../utils/date.model';
...
@Component({
    selector: 'pori-jalali-calendar',
    templateUrl: './jalali-calendar.html',
    styleUrls: ['./jalali-calendar.scss']
})
export class PoriJalaliCalendarComponent implements OnInit, OnChanges {
    ...
    generateCalendar() {
        ...
        this.days = this.rangeService.Range(start, start + numOfDaysInMonth).map((date: number, index: number): PoriDateModel => {
